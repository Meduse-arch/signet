...
export function Select({
  ...
  return (
    ...
    >
      {/* Libellé */}
      {label && (
        <span className="text-[10px] font-cinzel font-black text-gold-muted tracking-widest uppercase ml-1">
          {label}
        </span>
      )}
      ...
      <button
        ...
        className={[
          ...
          'text-xs font-cinzel transition-all duration-200',
          'disabled:opacity-50 disabled:cursor-not-allowed',
          isOpen
            ? 'border-gold-DEFAULT/60 ring-1 ring-gold-DEFAULT/20 text-gold-bright'
            : 'border-gold-DEFAULT/30 text-gold-bright hover:border-gold-DEFAULT/50',
        ].join(' ')}
      ...
      >
        <span className="flex items-center gap-2 tracking-widest uppercase truncate">
          ...
          {selectedOption ? selectedOption.label : (
            <span className="text-gold-muted/60 font-serif italic normal-case tracking-normal">{actualPlaceholder}</span>
          )}
        </span>
        <Icons.ChevronDown
          className={`w-4 h-4 text-gold-muted shrink-0 transition-transform duration-300 ${isOpen ? 'rotate-180 text-gold-bright' : ''}`}
        />
      </button>
      ...
      {isOpen && (
        <div
          ...
          className={[
            ...
            'bg-[#16161C] border border-gold-DEFAULT/40 rounded-xl',
        ...
        >
          {/* Recherche intégrée */}
          {searchable && (
            <div className="p-2 border-b border-gold-DEFAULT/10">
              <div className="relative">
                ...
                <input
                  ...
                  autoFocus
                  className="w-full bg-transparent pl-8 pr-3 py-2 text-[11px] font-serif italic text-gold-bright placeholder:text-gold-muted/40 outline-none"
                />
              </div>
            </div>
          )}
                  ...
                  <div
                    ...
                    className={[
                      ...
                      isSelected
                        ...
                        : 'text-gold-DEFAULT hover:text-gold-bright hover:bg-gold-DEFAULT/5',
                    ].join(' ')}
                  >
                    ...
                    {isSelected && (
                      <Icons.Check className="w-3.5 h-3.5 text-gold-bright shrink-0" />
                    )}
                  </div>
                );
              })
