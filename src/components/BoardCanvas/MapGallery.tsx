...
export function MapGallery({ maps, currentMapId, onSelectMap }: MapGalleryProps) {
 ...
 {isOpen && (
 <div className="mb-2 w-80 max-h-[400px] bg-black/80 backdrop-blur-xl border border-silver-DEFAULT/40 rounded-2xl shadow-2xl overflow-hidden flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-300">
 <div className="p-4 border-b border-white/10 flex items-center justify-between bg-white/5">
 <div className="flex items-center gap-2">
 <Grid className="w-4 h-4 text-glacier-bright" />
 <span className="text-xs font-quantico font-black text-glacier-bright tracking-widest uppercase">Galerie des Cartes</span>
 </div>
 <button 
 ...
 className="w-full bg-white/5 border border-white/10 rounded-lg py-2 pl-9 pr-4 text-xs text-white placeholder:text-white/60 focus:outline-none focus:border-silver-DEFAULT/30 transition-all"
 />
 </div>
 </div>
 ...
 <Search className="w-4 h-4 text-white/50 group-focus-within:text-glacier-bright transition-colors" />
 <input 
 type="text"
 ...
 className={`p-2 rounded-xl transition-all ${isOpen ? 'bg-glacier-DEFAULT text-black' : 'hover:bg-white/5 text-white/80 hover:text-glacier-bright'}`}
 ...
 <div className="h-12 px-5 bg-[#0D0D0F]/80 backdrop-blur-xl border border-silver-DEFAULT/40 rounded-2xl flex items-center gap-3 shadow-2xl">
 <div className="relative">
 <div className="absolute inset-0 bg-glacier-DEFAULT/20 blur-md rounded-full animate-pulse" />
 <MapIcon className="w-4 h-4 text-glacier-bright relative z-10" />
 </div>
 <div className="flex flex-col">
 <span className="text-[11px] font-black text-silver-bright drop-shadow-md uppercase tracking-[0.2em] leading-none mb-1">Localisation</span>
 <span className="text-xs font-quantico font-black text-white tracking-widest uppercase">
