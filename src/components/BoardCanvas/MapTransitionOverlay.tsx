...
export function MapTransitionOverlay({ progress, onRetry }: MapTransitionOverlayProps) {
  ...
  return (
    ...
    >
      ...
      <div className={`z-20 flex flex-col items-center gap-6 p-10 rounded-2xl bg-[#0D0D0F]/80 border border-gold-DEFAULT/30 shadow-[0_0_40px_rgba(0,0,0,0.9)] transition-all duration-1000
      ...
      `}>
        {progress.status === 'error' ? (
          <div className="flex flex-col items-center gap-4 pointer-events-auto">
            ...
            <p className="text-red-400 font-cinzel text-lg text-center max-w-sm">
              {message}
            </p>
            <button 
              onClick={onRetry}
              className="mt-4 flex items-center gap-2 px-6 py-2 rounded bg-gold-DEFAULT/20 text-gold-bright border border-gold-DEFAULT/50 hover:bg-gold-DEFAULT/40 transition-all font-cinzel uppercase text-sm tracking-widest active:scale-95"
            >
              <RefreshCcw size={16} />
          ...
          <>
            <div className="relative w-24 h-24 flex items-center justify-center">
              <div className="absolute inset-0 border-4 border-gold-DEFAULT/10 rounded-full"></div>
              <div className="absolute inset-0 border-4 border-gold-DEFAULT rounded-full border-t-transparent animate-spin"></div>
              ...
              {progress.status !== 'waiting_manifest' && (
                <span className="text-gold-bright font-cinzel font-bold text-lg">{percentage}%</span>
              )}
            </div>
            <div className="flex flex-col items-center gap-2 text-center">
              <p className="text-white font-cinzel text-2xl tracking-widest uppercase text-glow-gold">
                {message}
              </p>
