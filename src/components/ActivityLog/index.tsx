    ...
    >
      ...
      <div className="flex-1 min-w-0 flex flex-col gap-0.5">
        ...
        <div className="flex items-center justify-between gap-2">
          {log.character_name && (
            <span
              className="text-[10px] font-cinzel font-black uppercase tracking-widest truncate"
        ...
        {/* Action principale */}
        <span className="text-xs font-cinzel text-white/80 uppercase tracking-wide leading-tight">
          {log.action}
        </span>
        ...
        {log.type === 'des' && log.details?.total !== undefined && (
          <div className="flex items-center gap-2 mt-0.5">
            ...
            <span
              className="text-base font-cinzel font-black leading-none"
              style={{ color: log.color }}
            >
              = {log.details.total}
            </span>
...
export function ActivityLogPanel({ sessionId, onClose }: ActivityLogPanelProps) {
  ...
  return (
    <motion.div
      ...
      transition={{ duration: 0.2 }}
      className="flex flex-col w-[380px] max-h-[560px] bg-[#0D0D0F]/95 border border-gold-DEFAULT/30 rounded-2xl overflow-hidden shadow-[0_0_40px_rgba(0,0,0,0.8)] backdrop-blur-2xl"
    >
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-gold-DEFAULT/20 bg-black/40 shrink-0">
        <div className="flex items-center gap-2">
          <ScrollText size={14} className="text-gold-DEFAULT" />
          <span className="text-xs font-cinzel font-black text-gold-bright uppercase tracking-widest">
            Annales de Session
          </span>
      ...
      <div className="flex gap-1 px-3 py-2 overflow-x-auto no-scrollbar border-b border-white/5 shrink-0">
        {ALL_FILTERS.map(f => (
          <button
            ...
            className={`px-2.5 py-1 rounded-full text-[10px] font-cinzel font-black uppercase tracking-widest border whitespace-nowrap transition-all ${
              filter === f.type
        ...
        <AnimatePresence initial={false}>
          {filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 opacity-20 gap-3">
              <ScrollText size={32} className="text-gold-DEFAULT" />
      ...
      {logs.length > 0 && (
        <div className="px-3 py-2 border-t border-white/5 bg-black/20 flex items-center justify-between shrink-0">
          ...
          <button
            ...
            className="text-[9px] font-cinzel uppercase tracking-widest text-gold-DEFAULT/40 hover:text-gold-DEFAULT transition-colors"
          >
            Ouvrir en fenêtre →
          </button>
        </div>
...
export function ActivityLogWindowContent({ sessionId }: { sessionId: string }) {
  ...
  return (
    <div className="flex flex-col h-full bg-[#0D0D0F] text-white">
      {/* Header */}
      <div className="flex items-center gap-3 px-4 py-3 border-b border-gold-DEFAULT/20 bg-black/40 shrink-0">
        <ScrollText size={16} className="text-gold-DEFAULT" />
        <span className="text-sm font-cinzel font-black text-gold-bright uppercase tracking-widest">
          Annales de Session
        </span>
        <div className="ml-auto flex items-center gap-2">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[9px] font-cinzel uppercase tracking-widest text-white/40">Live</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-1.5 px-4 py-2.5 overflow-x-auto no-scrollbar border-b border-white/5 shrink-0 bg-black/20">
        {ALL_FILTERS.map(f => (
          <button
            key={f.type}
            onClick={() => setFilter(f.type as LogType | 'all')}
            className={`px-3 py-1.5 rounded-full text-[11px] font-cinzel font-black uppercase tracking-widest border whitespace-nowrap transition-all ${
              filter === f.type
                ? 'bg-gold-DEFAULT text-black border-gold-DEFAULT'
                : 'bg-white/5 border-white/10 text-white/50 hover:border-white/20 hover:text-white/70'
            }`}
        ...
        <AnimatePresence initial={false}>
          {filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full opacity-20 gap-4">
              <ScrollText size={48} className="text-gold-DEFAULT" />
