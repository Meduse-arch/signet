...
export function QuestsWindowContent({ sessionId }: QuestsWindowContentProps) {
 ...
 default: return <Clock className="text-silver-bright animate-pulse" size={14} />;
 }
 };
 ...
 className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-md text-[11px] font-quantico font-black uppercase tracking-widest transition-all ${activeTab === 'current' ? 'bg-glacier-DEFAULT text-black' : 'text-white/60 hover:text-white hover:bg-white/5'}`}
 >
 <Clock size={10} /> {t('context.inProgress', 'En cours')}
 </button>
 <button 
 onClick={() => setActiveTab('completed')}
 className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-md text-[11px] font-quantico font-black uppercase tracking-widest transition-all ${activeTab === 'completed' ? 'bg-glacier-DEFAULT text-black' : 'text-white/60 hover:text-white hover:bg-white/5'}`}
 >
 ...
 <Scroll size={18} className="text-white/10 group-hover:text-silver-bright/40 transition-colors" />
 )}
 ...
 <h4 className={`text-xs font-quantico font-black tracking-widest truncate uppercase ${isActive ? 'text-glacier-bright' : 'text-white/60 group-hover:text-white'}`}>
 {quest.title}
 </h4>
 <p className="text-xs font-inter italic text-white/50 truncate mt-0.5">
 ...
 <span className="text-xs font-quantico font-black tracking-[0.4em] uppercase">{t('context.worldChronicles', "Chroniques du Monde")}</span>
 </div>
 )}
 </div>
 )}
