...
export const InitiativeWindowContent = ({ sessionId }: { sessionId: string }) => {
 ...
 <h2 className="font-quantico text-xl text-silver-bright tracking-wider">{t('context.combatOrder', 'Ordre de Combat')}</h2>
 </div>
 ...
 {isActive && <span className="font-bold text-glacier-bright">{t('context.round', 'Round')} {currentRound}</span>}
 </div>

 <div className="flex-1 overflow-y-auto mb-4 bg-black/20 rounded-xl p-2 border border-white/5 shadow-inner">
 {actors.map((actor) => (
 <div key={actor.id} className={`flex items-center gap-3 p-2 mb-2 rounded-lg border transition-all duration-300 ${actor.id === activeActorId ? 'border-silver-DEFAULT bg-glacier-DEFAULT/10 shadow-[0_0_15px_rgba(240,192,64,0.15)] scale-[1.02]' : 'border-white/5 bg-black/40'}`}>
 ...
 <div className="w-10 h-10 rounded-full border border-gray-700 bg-gray-900 flex items-center justify-center font-quantico text-xs text-gray-500">?</div>
 )}
 ...
 <div className={`font-bold truncate ${actor.id === activeActorId ? 'text-glacier-bright' : 'text-gray-300'}`}>{actor.name}</div>
 </div>
 </div>
 ))}
 {actors.length === 0 && <div className="text-center text-gray-600 mt-8 font-quantico tracking-wider text-xs">{t('context.noFighter', 'Aucun combattant')}</div>}
 </div>
 </div>
 );
 }
 ...
 <div className="flex flex-col h-full bg-[#0D0D0F] text-gray-300 p-4 font-inter text-sm">
 <div className="flex justify-between items-center mb-6 border-b border-white/5 pb-4">
 <h2 className="font-quantico text-xl text-silver-bright tracking-wider">{t('context.combatManagement', 'Gestion des Combats')}</h2>
 ...
 <select onChange={(e) => { if(e.target.value) handleAddCharacter(e.target.value); e.target.value=''; }} className="bg-black/40 border border-white/10 rounded-lg p-2 flex-1 outline-none focus:border-silver-DEFAULT/50 text-gray-300">
 ...
 className="w-12 bg-black/40 border border-white/10 rounded text-center text-gray-300 focus:border-silver-DEFAULT"
 />
 </div>
 </div>
 ...
 {isActive && (
 <button onClick={handleNextTurn} className="w-full py-4 mt-auto bg-gradient-to-r from-gold-DEFAULT/10 via-gold-DEFAULT/20 to-gold-DEFAULT/10 text-glacier-bright border border-silver-DEFAULT/50 rounded-xl hover:from-gold-DEFAULT/20 hover:via-gold-DEFAULT/30 hover:to-gold-DEFAULT/20 transition-all font-quantico font-bold tracking-[0.2em] uppercase shadow-[0_0_20px_rgba(240,192,64,0.15)] flex justify-center items-center gap-2">
