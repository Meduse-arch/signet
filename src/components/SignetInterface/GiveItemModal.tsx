...
export function GiveItemModal({ sessionId, targetCharacterName, onClose, onGive }: GiveItemModalProps) {
 ...
 <div className="relative w-full max-w-2xl bg-[#0D0D0F] border border-silver-DEFAULT/30 rounded-2xl overflow-hidden flex flex-col max-h-[80vh] animate-in zoom-in-95 duration-200" style={{ boxShadow: 'inset 0 0 20px rgba(212, 175, 55, 0.1), 0 20px 50px rgba(0, 0, 0, 0.5)' }}>
 ...
 <button onClick={onClose} className="p-1 hover:bg-glacier-DEFAULT/10 rounded-full transition-colors text-silver-bright/60 hover:text-glacier-bright">
 <X size={18} />
 </button>
 </div>
 ...
 {/* Liste des items */}
 <div className="w-2/3 border-r border-silver-DEFAULT/10 flex flex-col bg-black/20">
 <div className="p-4 border-b border-silver-DEFAULT/10">
 <div className="relative">
 <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-silver-bright/40" />
 <input 
 type="text" 
 placeholder="Chercher un artefact..."
 value={search}
 onChange={(e) => setSearch(e.target.value)}
 className="w-full bg-black/40 border border-silver-DEFAULT/20 rounded-xl py-2 pl-9 pr-4 text-xs font-quantico text-glacier-bright placeholder:text-silver-bright/40 focus:outline-none focus:border-silver-DEFAULT/50 transition-all"
 />
 </div>
 </div>
 ...
 >
 <div className="w-10 h-10 rounded-lg bg-black/60 border border-silver-DEFAULT/20 flex items-center justify-center shrink-0">
 {item.image_url ? (
 <AssetImage src={item.image_url} alt="" className="w-full h-full object-cover rounded-lg" />
 ) : (
 <Package size={20} className="text-silver-bright/50" />
 )}
 </div>
 <div className="flex-1 min-w-0">
 <div className="text-xs font-quantico font-black text-glacier-bright truncate uppercase">{item.name}</div>
 <div className="text-xs text-white/60 italic truncate">{item.category}</div>
 </div>
 </button>
 ))}
 {filteredItems.length === 0 && (
 <div className="text-center py-8 text-white/50 text-xs italic font-inter">Aucun artefact trouvé dans la bibliothèque de session.</div>
 )}
 </div>
 </div>
 ...
 <Package size={32} className="text-silver-bright/50" />
 )}
 </div>
 <h3 className="text-sm font-quantico font-black text-glacier-bright text-center uppercase tracking-widest mb-1">{selectedItem.name}</h3>
 <div className="text-xs text-center text-white/60 italic mb-4">{selectedItem.category}</div>
 
 <p className="text-xs text-white/60 font-inter leading-relaxed line-clamp-4">
 {selectedItem.description}
 </p>
 </div>

 <div className="mt-4 pt-4 border-t border-silver-DEFAULT/20">
 <label className="block text-xs font-quantico text-silver-bright/70 uppercase tracking-widest text-center mb-2">Quantité à transférer</label>
 ...
 <span className="font-quantico font-black text-lg text-white w-8 text-center">{quantity}</span>
 <button 
 ...
 <span className="font-quantico text-xs uppercase tracking-widest">Sélectionnez un artefact</span>
 </div>
 )}
 </div>
 </div>
