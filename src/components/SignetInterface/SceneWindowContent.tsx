...
export function SceneWindowContent({ sessionId, scenes, currentSceneId, onSelectScene, onAddScene, onUpdateScene, onToggleHide, onRemoveScene }: SceneWindowContentProps) {
 ...
 ? 'border-silver-DEFAULT shadow-[0_0_15px_rgba(157, 168, 184,0.4)] scale-[1.02]' 
 : 'border-white/10 hover:border-silver-DEFAULT/50 hover:scale-[1.01]'
 ...
 <span className={`text-xs font-quantico font-black transition-colors ${
 currentSceneId === scene.id ? 'text-glacier-bright' : 'text-silver-bright drop-shadow-md/50'
 }`}>
 {(index + 1).toString().padStart(2, '0')}
 </span>
 <div className="flex flex-col flex-1 truncate">
 <span className="text-xs font-quantico font-bold text-white tracking-widest uppercase truncate pr-8">
 {scene.name}
 </span>
 <div className="flex gap-2 items-center">
 {isMJ && scene.is_hidden && scene.id !== 'initial-scene' && (
 <span className="text-[11px] font-quantico font-black text-red-500 uppercase tracking-widest">{t('context.sceneHidden', 'Cachée')}</span>
 )}
 <span className="text-[11px] font-quantico font-black text-white/50 uppercase tracking-widest">{t('context.gridSize', 'Grille:')} {scene.grid_size || 50}px</span>
 </div>
 </div>
 </div>
 ...
 className="p-2 rounded-full bg-black/40 border border-white/10 text-white/60 hover:text-glacier-bright hover:border-silver-DEFAULT/40 transition-all"
 ...
 {currentSceneId === scene.id && (
 <div className="absolute right-4 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full border border-silver-DEFAULT/50 flex items-center justify-center">
 <div className="w-4 h-4 rounded-full bg-glacier-DEFAULT/40 animate-ping absolute" />
 <div className="w-4 h-4 rounded-full bg-glacier-DEFAULT/60 relative z-10" />
 </div>
 )}
 </div>
 ))}
 ...
 <form onSubmit={editingId ? handleUpdateSubmit : handleAddSubmit} className="p-3 rounded-lg border border-silver-DEFAULT/40 bg-[#0D0D0F]/80 space-y-3 animate-in fade-in zoom-in-95 duration-200">
 <h4 className="text-xs font-quantico font-black text-glacier-bright uppercase tracking-widest border-b border-white/5 pb-1">
 ...
 className="flex-1 py-1.5 rounded bg-white/5 text-[11px] font-quantico text-white/80 hover:bg-white/10"
 >
 ...
 className="flex-1 py-1.5 rounded bg-glacier-DEFAULT/20 text-[11px] font-quantico text-glacier-bright border border-silver-DEFAULT/30 hover:bg-glacier-DEFAULT/30 disabled:opacity-50 disabled:cursor-not-allowed"
 >
 ...
 className="w-full py-3 rounded-lg border border-dashed border-silver-DEFAULT/30 bg-glacier-DEFAULT/5 hover:bg-glacier-DEFAULT/10 text-silver-bright drop-shadow-md hover:text-glacier-bright transition-all flex items-center justify-center gap-2 group"
 >
 <Plus size={16} className="group-hover:rotate-90 transition-transform" />
 <span className="text-xs font-quantico font-black tracking-widest uppercase">{t('context.newSceneBtn', "Nouvelle Scène")}</span>
 </button>
 )}
 </div>
 );
