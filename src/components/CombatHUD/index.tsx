...
export const CombatHUD = ({ sessionId }: { sessionId: string }) => {
 ...
 <div 
 onClick={onClick}
 className={`relative rounded-full flex-shrink-0 cursor-pointer overflow-hidden bg-[#1A1A20] transition-all duration-300 border ${isActive ? 'border-silver-DEFAULT shadow-[0_0_15px_rgba(240,192,64,0.2)] scale-110 z-10' : 'border-white/10 opacity-60 hover:opacity-100 hover:scale-105'}`}
 >
 <div className={`${size}`}>
 ...
 <span className="font-quantico font-bold tracking-widest text-sm text-gray-200">{currentActor.name}</span>
 )}
 </div>
 </div>
 )}
 ...
 <button 
 onClick={handleNext}
 className="flex items-center justify-center w-10 h-10 rounded-full bg-black/40 border border-silver-DEFAULT/20 text-silver-bright hover:bg-glacier-DEFAULT/10 hover:border-silver-DEFAULT/50 transition-all"
 ...
 className={`flex items-center justify-center w-10 h-10 rounded-full transition-all border ${isActive ? 'bg-red-900/20 border-red-500/20 text-red-400 hover:bg-red-900/40 hover:border-red-500/50' : 'bg-black/40 border-silver-DEFAULT/20 text-silver-bright hover:bg-glacier-DEFAULT/10 hover:border-silver-DEFAULT/50'}`}
