        ...
        <div className="relative z-10 flex flex-col h-full overflow-hidden">
          ...
          <div className="flex border-b border-gold-DEFAULT/10 px-4 sm:px-8 shrink-0 overflow-x-auto no-scrollbar">
            <button 
              ...
              className={`px-4 py-3 text-xs sm:text-[11px] font-cinzel font-black tracking-widest uppercase transition-all border-b-2 whitespace-nowrap ${activeTab === 'creation' ? 'border-gold-DEFAULT text-gold-bright' : 'border-transparent text-gold-muted hover:text-gold-DEFAULT'}`}
            >
              Création
            </button>
            <button 
              onClick={() => setActiveTab('stats')}
              className={`px-4 py-3 text-xs sm:text-[11px] font-cinzel font-black tracking-widest uppercase transition-all border-b-2 whitespace-nowrap ${activeTab === 'stats' ? 'border-gold-DEFAULT text-gold-bright' : 'border-transparent text-gold-muted hover:text-gold-DEFAULT'}`}
            >
              Attributs
            </button>
            <button 
              onClick={() => setActiveTab('bars')}
              className={`px-4 py-3 text-xs sm:text-[11px] font-cinzel font-black tracking-widest uppercase transition-all border-b-2 whitespace-nowrap ${activeTab === 'bars' ? 'border-gold-DEFAULT text-gold-bright' : 'border-transparent text-gold-muted hover:text-gold-DEFAULT'}`}
            >
              Ressources
            </button>
          </div>
                ...
                <div className="flex flex-col gap-4 sm:gap-6">
                  ...
                  <div className="flex gap-2 p-1 bg-black/40 rounded-xl border border-gold-DEFAULT/20 w-fit">
                    <button
                      ...
                      className={`px-4 sm:px-6 py-2 rounded-lg text-xs sm:text-[11px] font-cinzel font-black tracking-widest uppercase transition-all ${
                      ...
                      <div className="space-y-6 animate-in slide-in-from-left-4 duration-500">
                        <div className="flex flex-col items-center gap-4">
                          ...
                          <div className="text-center">
                            ...
                            <p className="mt-4 text-[11px] text-gold-muted/50 font-serif italic italic leading-relaxed">
                      ...
                      <div className="space-y-6 sm:space-y-8 animate-in slide-in-from-right-4 duration-500">
                        <div className="flex flex-col items-center gap-4">
                          ...
                          <div className="text-center w-full">
                            ...
                            <p className="mt-8 text-[11px] text-gold-muted/50 font-serif italic italic leading-relaxed max-w-sm mx-auto">
              ...
              <div className="space-y-4 animate-in fade-in duration-300">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button 
                    ...
                    className="flex items-center gap-2 px-3 py-1 rounded-full bg-gold-DEFAULT/10 border border-gold-DEFAULT/30 text-xs font-cinzel font-black text-gold-bright hover:bg-gold-DEFAULT/20 transition-all"
                  >
                ...
                <div className="grid grid-cols-1 gap-2">
                  {localSettings.stats?.map((stat) => (
                    <div key={stat.id} className="flex items-center gap-3 p-2 sm:p-3 rounded-xl bg-white/5 border border-white/5 group">
                      ...
                      />
                      <span className="text-xs sm:text-[11px] font-mono text-white/60 uppercase px-2">{stat.id}</span>
                      <button 
              ...
              <div className="space-y-6 animate-in fade-in duration-300">
                ...
                {localSettings.bars?.map((bar) => (
                  <div key={bar.id} className="p-4 sm:p-5 rounded-2xl bg-white/5 border border-white/10 space-y-4 relative group">
                    ...
                    <div className="space-y-1">
                      <label className="text-xs font-mono text-white/50 uppercase tracking-tighter">Équation de calcul</label>
                      <input 
                        type="text"
                        value={bar.formula}
          ...
          <footer className="p-4 sm:p-8 border-t border-gold-DEFAULT/10 shrink-0">
            <button
              ...
              className="w-full py-3 sm:py-4 flex items-center justify-center gap-3 bg-gold-DEFAULT text-black rounded-2xl border-2 border-gold-DEFAULT hover:bg-gold-bright hover:shadow-[0_0_30px_rgba(212,175,55,0.4)] transition-all group"
            >
              <Save className="w-4 h-4 group-hover:scale-110 transition-transform" />
              <span className="text-xs font-cinzel font-black tracking-[0.2em] uppercase">Enregistrer</span>
            </button>
          </footer>
        </div>
      </div>
