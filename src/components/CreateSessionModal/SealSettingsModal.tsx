...
export function SealSettingsModal({ isOpen, onClose, settings, onSave }: SealSettingsModalProps) {
 ...
 maxWidth="2xl"
 >
 <div className="-mx-6 border-b border-silver-DEFAULT/10 mb-4 px-6 flex overflow-x-auto no-scrollbar">
 <button 
 onClick={() => setActiveTab('creation')}
 className={`px-4 py-3 text-[11px] font-quantico font-black tracking-widest uppercase transition-all border-b-2 whitespace-nowrap ${activeTab === 'creation' ? 'border-silver-DEFAULT text-glacier-bright' : 'border-transparent text-glacier-muted hover:text-silver-bright'}`}
 >
 {t('seal.creation', 'Création')}
 </button>
 <button 
 onClick={() => setActiveTab('stats')}
 className={`px-4 py-3 text-[11px] font-quantico font-black tracking-widest uppercase transition-all border-b-2 whitespace-nowrap ${activeTab === 'stats' ? 'border-silver-DEFAULT text-glacier-bright' : 'border-transparent text-glacier-muted hover:text-silver-bright'}`}
 >
 {t('seal.attributes', 'Attributs')}
 </button>
 <button 
 onClick={() => setActiveTab('bars')}
 className={`px-4 py-3 text-[11px] font-quantico font-black tracking-widest uppercase transition-all border-b-2 whitespace-nowrap ${activeTab === 'bars' ? 'border-silver-DEFAULT text-glacier-bright' : 'border-transparent text-glacier-muted hover:text-silver-bright'}`}
 >
 ...
 <Icons.ScrollText className="w-4 h-4 text-glacier-muted" />
 <h3 className="text-xs font-quantico font-black text-glacier-muted tracking-widest uppercase">{t('seal.creationMethod', 'Méthode de Création')}</h3>
 </div>

 <div className="flex gap-2 p-1 bg-black/40 rounded-xl border border-silver-DEFAULT/20 w-fit">
 <button
 onClick={() => setLocalSettings({ ...localSettings, sheetMode: 'manual' })}
 className={`px-6 py-2 rounded-lg text-[11px] font-quantico font-black tracking-widest uppercase transition-all ${
 localSettings.sheetMode === 'manual'
 ? 'bg-glacier-DEFAULT text-black shadow-rune-gold'
 : 'text-glacier-muted hover:text-silver-bright'
 }`}
 >
 ...
 <div className="p-8 bg-[#0D0D0F]/80 border border-silver-DEFAULT/30 rounded-2xl relative overflow-hidden group">
 <div className="absolute inset-0 bg-glacier-DEFAULT/2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none" />
 
 {localSettings.sheetMode === 'manual' ? (
 <div className="space-y-6 animate-in slide-in-from-left-4 duration-500">
 <div className="flex flex-col items-center gap-4">
 <Icons.UserCog className="w-8 h-8 text-glacier-bright opacity-50" />
 <div className="text-center">
 <label className="block text-xs font-quantico font-black text-glacier-muted tracking-widest uppercase mb-4">{t('seal.pointPool', 'Réserve de Points')}</label>
 <div className="flex items-center justify-center gap-6">
 <button 
 onClick={() => setLocalSettings({ ...localSettings, manualPoints: Math.max(0, (localSettings.manualPoints || 0) - 5) })}
 className="w-10 h-10 rounded-full border border-silver-DEFAULT/30 flex items-center justify-center text-silver-bright hover:bg-glacier-DEFAULT/10 transition-all"
 >
 -5
 </button>
 <Input
 ...
 className="text-center font-quantico font-black text-2xl !px-1"
 ...
 <p className="mt-4 text-[11px] text-glacier-muted/50 font-inter italic leading-relaxed">
 ...
 <div className="flex flex-col items-center gap-4">
 <Icons.Dices className="w-8 h-8 text-glacier-bright opacity-50" />
 <div className="text-center w-full">
 <label className="block text-xs font-quantico font-black text-glacier-muted tracking-widest uppercase mb-6 text-center">{t('seal.rollFormula', 'Formule de Tirage')}</label>
 
 <div className="flex items-center justify-center gap-6 text-glacier-bright font-quantico font-black">
 <div className="flex flex-col items-center gap-2">
 <span className="text-xs text-glacier-muted uppercase tracking-tighter">{t('seal.number', 'Nombre')}</span>
 <Input
 type="number"
 ...
 className="text-center text-xl font-quantico !px-1"
 wrapperClassName="w-16"
 />
 </div>
 <span className="text-4xl mt-4 opacity-30">D</span>
 <div className="flex flex-col items-center gap-2">
 <span className="text-xs text-glacier-muted uppercase tracking-tighter">{t('seal.sides', 'Faces')}</span>
 <Input
 type="number"
 ...
 <div className="w-px h-8 bg-glacier-DEFAULT/20 mx-2 self-end mb-4" />
 <div className="flex flex-col items-center gap-2">
 <span className="text-xs text-glacier-muted uppercase tracking-tighter">{t('seal.rerolls', 'Relances')}</span>
 <Input
 type="number"
 ...
 <p className="mt-8 text-[11px] text-glacier-muted/50 font-inter italic leading-relaxed max-w-sm mx-auto">
 {t('seal.rollFormulaDesc', "Définit le jet de dés automatique lors de la création d'un personnage.")}
 </p>
 </div>
 </div>

 <div className="mt-6 pt-6 border-t border-silver-DEFAULT/10 flex items-center justify-between">
 <div className="space-y-1">
 <h4 className="text-xs font-quantico font-black text-glacier-bright tracking-widest uppercase">{t('seal.rerollAll', 'Relancer tout')}</h4>
 <p className="text-xs text-glacier-muted font-inter italic">{t('seal.rerollAllDesc', "Permettre de relancer toute la fiche d'un coup.")}</p>
 </div>
 <button
 ...
 ? 'bg-glacier-DEFAULT/20 border-silver-DEFAULT' 
 : 'bg-black/40 border-silver-DEFAULT/20'
 }`}
 >
 <div className={`absolute top-1 w-3.5 h-3.5 rounded-full transition-all ${
 localSettings.rerollAllAllowed 
 ? 'right-1 bg-glacier-bright shadow-rune-gold' 
 : 'left-1 bg-glacier-muted'
 }`} />
 </button>
 </div>
 </div>
 ...
 <h3 className="text-xs font-quantico font-black text-glacier-muted tracking-widest uppercase">{t('seal.customAttributes', 'Attributs Personnalisés')}</h3>
 <Button 
 size="sm"
 variant="ghost"
 ...
 <div className="flex justify-between items-center mb-4">
 <h3 className="text-xs font-quantico font-black text-glacier-muted tracking-widest uppercase">{t('seal.formulas', 'Formules de Calcul (Ressources)')}</h3>
 <Button 
 size="sm"
 variant="ghost"
 leftIcon={<Icons.Plus size={14} />}
 ...
 <p className="text-[11px] text-blue-300/70 font-inter italic leading-relaxed">
