...
export function CreateCharacterModal({ 
 ...
 >
 <div className="relative w-full max-w-5xl bg-[#0D0D0F]/95 backdrop-blur-2xl border border-silver-DEFAULT/30 rounded-[1.5rem] sm:rounded-[2.5rem] shadow-[0_0_50px_rgba(0,0,0,0.8)] overflow-hidden flex flex-col max-h-[95vh] sm:max-h-[90vh]">
 {/* Golden Corners */}
 <div className="absolute top-0 left-0 w-8 h-8 border-t-2 border-l-2 border-silver-DEFAULT/50 rounded-tl-[1.5rem] sm:rounded-tl-[2.5rem] pointer-events-none" />
 <div className="absolute bottom-0 right-0 w-8 h-8 border-b-2 border-r-2 border-silver-DEFAULT/50 rounded-br-[1.5rem] sm:rounded-br-[2.5rem] pointer-events-none" />
 
 <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 sm:p-6 border-b border-silver-DEFAULT/10 gap-4 shrink-0">
 <div className="flex-1">
 <h2 className="text-lg sm:text-xl font-quantico font-black text-glacier-bright tracking-[0.2em] uppercase drop-shadow-md">
 {title}
 </h2>
 <div className="mt-1">
 {!isMJ && (
 mode === 'manual' ? (
 <p className="text-[11px] sm:text-xs font-quantico text-silver-bright/60 tracking-widest uppercase">
 Points restants: <span className="text-glacier-bright font-black">{pointsLeft}</span>
 </p>
 ) : (
 <p className="text-[11px] sm:text-xs font-quantico text-silver-bright/60 tracking-widest uppercase">
 Faveurs (Rerolls): <span className="text-glacier-bright font-black">{rerollsLeft}</span>
 </p>
 )
 )}
 </div>
 ...
 className="flex-1 sm:flex-none px-3 sm:px-4 py-2 rounded-xl bg-glacier-DEFAULT/10 border border-silver-DEFAULT/30 text-xs sm:text-[11px] font-quantico font-black text-glacier-bright hover:bg-glacier-DEFAULT/20 transition-all tracking-widest uppercase flex items-center justify-center gap-2"
 >
 <Dices size={14} />
 ...
 {isMJ && (
 <div className="bg-black/20 border-b border-silver-DEFAULT/10 px-4 sm:px-6 py-3 flex flex-wrap items-center gap-4 shrink-0">
 <div className="flex items-center gap-2 bg-white/5 p-1 rounded-lg border border-silver-DEFAULT/20 shadow-inner">
 <button 
 onClick={() => setMode('manual')}
 className={`px-4 py-1.5 text-[11px] font-quantico font-black rounded-md transition-all uppercase tracking-widest ${mode === 'manual' ? 'bg-glacier-DEFAULT text-black shadow-md' : 'text-silver-bright/40 hover:text-silver-bright'}`}
 >
 Manuel
 </button>
 <button 
 onClick={() => setMode('roll')}
 className={`px-4 py-1.5 text-[11px] font-quantico font-black rounded-md transition-all uppercase tracking-widest ${mode === 'roll' ? 'bg-glacier-DEFAULT text-black shadow-md' : 'text-silver-bright/40 hover:text-silver-bright'}`}
 >
 Tirage
 </button>
 </div>

 <label className="flex items-center gap-3 cursor-pointer group bg-white/5 px-4 py-2 rounded-lg border border-silver-DEFAULT/20 hover:border-silver-DEFAULT/40 transition-all">
 <input 
 type="checkbox" 
 checked={isTemplate} 
 ...
 className="sr-only"
 />
 <div className={`w-8 h-4 rounded-full border border-silver-DEFAULT/30 transition-all flex items-center p-0.5 ${isTemplate ? 'bg-purple-500/20 border-purple-500/50' : 'bg-black/40'}`}>
 <div className={`w-3 h-3 rounded-full transition-all ${isTemplate ? 'translate-x-3.5 bg-purple-400 shadow-[0_0_10px_rgba(168,85,247,0.5)]' : 'translate-x-0 bg-glacier-DEFAULT/40'}`} />
 </div>
 <span className={`text-xs font-quantico font-black uppercase tracking-widest ${isTemplate ? 'text-purple-400' : 'text-silver-bright/60'}`}>
 Modèle Neutre
 </span>
 </label>

 {!isTemplate && (
 <div className="flex items-center gap-1 bg-white/5 p-1 rounded-lg border border-silver-DEFAULT/20 ml-auto">
 {(['Joueur', 'PNJ', 'Monstre', 'Boss'] as const).map(t => (
 <button 
 key={t}
 onClick={() => setType(t)}
 className={`px-3 py-1.5 text-[11px] font-quantico font-black rounded-md transition-all uppercase tracking-widest ${
 type === t 
 ? (t === 'Boss' ? 'bg-red-500 text-white shadow-md' : t === 'Monstre' ? 'bg-orange-500 text-white shadow-md' : 'bg-blue-500 text-white shadow-md') 
 : 'text-silver-bright/40 hover:text-silver-bright'
 }`}
 >
 {t}
 </button>
 ...
 <label className="text-xs font-quantico font-black text-silver-bright tracking-widest uppercase ml-1">Nom de l'Entité</label>
 ...
 className="flex-1 bg-white/5 border border-silver-DEFAULT/20 rounded-xl px-4 py-2 text-white placeholder:text-white/60 focus:outline-none focus:border-silver-DEFAULT/50 transition-colors font-inter italic text-sm disabled:opacity-50 w-full"
 />
 ...
 className="w-16 shrink-0 bg-white/5 border border-silver-DEFAULT/20 rounded-xl px-2 py-2 text-center text-white focus:outline-none focus:border-silver-DEFAULT/50 transition-colors font-mono text-sm [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
 ...
 <div className="space-y-2">
 <label className="text-xs font-quantico font-black text-silver-bright tracking-widest uppercase ml-1">Image de l'Entité (URL)</label>
 <div className="flex gap-2">
 <input 
 type="text" 
 value={imageUrl}
 ...
 className="flex-1 bg-white/5 border border-silver-DEFAULT/20 rounded-xl px-4 py-2 text-white placeholder:text-white/60 focus:outline-none focus:border-silver-DEFAULT/50 transition-colors font-inter italic text-sm"
 />
 <button 
 type="button"
 ...
 <p className="text-[10px] font-quantico text-white/40 uppercase tracking-widest px-1">Importez un portrait local (P2P)</p>
 </div>
 </div>

 <div className="space-y-4">
 <h3 className="text-xs font-quantico font-black text-silver-bright/60 tracking-[0.2em] uppercase border-b border-silver-DEFAULT/10 pb-2">Attributs Primordiaux</h3>
 <div className="grid grid-cols-1 gap-2">
 {availableStats.map(stat => (
 <div key={stat.id} className="flex items-center justify-between bg-white/[0.02] p-2 rounded-lg border border-white/5 hover:border-silver-DEFAULT/20 transition-colors">
 <div className="flex flex-col">
 <span className="text-xs font-quantico text-white/80 uppercase tracking-widest">{stat.name}</span>
 </div>
 ...
 <button onClick={() => handleStatChange(stat.id, stats[stat.id] - 1)} className="w-6 h-6 flex items-center justify-center rounded bg-glacier-DEFAULT/10 text-glacier-bright hover:bg-glacier-DEFAULT/20 transition-colors">-</button>
 <span className="w-8 text-center font-quantico font-black text-lg text-glacier-bright drop-shadow-md">{stats[stat.id]}</span>
 <button onClick={() => handleStatChange(stat.id, stats[stat.id] + 1)} className="w-6 h-6 flex items-center justify-center rounded bg-glacier-DEFAULT/10 text-glacier-bright hover:bg-glacier-DEFAULT/20 transition-colors">+</button>
 </div>
 ) : (
 <>
 <span className="w-8 text-center font-quantico font-black text-lg text-glacier-bright drop-shadow-md">
 {stats[stat.id] || '?'}
 </span>
 <button 
 onClick={() => handleRerollStat(stat.id)} 
 disabled={!isMJ && rerollsLeft <= 0}
 className="w-8 h-8 flex items-center justify-center rounded-lg bg-glacier-DEFAULT/10 text-glacier-bright hover:bg-glacier-DEFAULT/20 disabled:opacity-40 transition-all border border-silver-DEFAULT/20"
 ...
 <h3 className="text-xs font-quantico font-black text-silver-bright/60 tracking-[0.2em] uppercase border-b border-silver-DEFAULT/10 pb-2 text-center">Énergie Vitale Dérivée</h3>
 
 <div className="space-y-4 bg-black/40 p-6 rounded-[2rem] border border-silver-DEFAULT/10 shadow-inner">
 {(settings?.bars || DEFAULT_BARS).map(bar => (
 <div key={bar.id} className="space-y-2">
 <div className="flex items-center justify-between text-[11px] font-quantico uppercase tracking-widest min-w-0" style={{ color: bar.color }}>
 ...
 <div className="p-4 bg-glacier-DEFAULT/5 border border-dashed border-silver-DEFAULT/20 rounded-2xl hidden sm:block">
 <p className="text-xs text-silver-bright/50 font-inter italic leading-relaxed text-center">
 ...
 <footer className="p-4 sm:p-6 border-t border-silver-DEFAULT/10 flex justify-end shrink-0">
 <button 
 onClick={handleSave}
