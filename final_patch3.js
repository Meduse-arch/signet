...
// 2. Fix the Tabs array - force replace
const oldTabs = "{ id: 'inventaire', label: 'INVENTAIRE', icon: Package },";
const newTabs = `{ id: 'inventaire', label: 'INVENTAIRE', icon: Package },
              ...(isMJ ? [
                { id: 'competences', label: 'SKILLS', icon: Zap },
                { id: 'quetes', label: 'QUÊTES', icon: Target }
              ] : [])`;
// Replace only the first occurrence to avoid messing up anything else
...
if (idxStart !== -1 && idxEnd !== -1) {
    const questsUI = `{activeTab === 'quetes' && (
          ...
          `;
    content = content.slice(0, idxStart) + questsUI + content.slice(idxEnd); // RESTORED content.slice(idxEnd) to KEEP the footer!
}
...
console.log('Final patch applied successfully v3!');
